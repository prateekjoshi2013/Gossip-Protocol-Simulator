...
case class sendPostStopState(i: Int)
object GossipSimulatorFailureModel extends App {

  override def main(args: Array[String]) {

    println("enter the parameters")
    //   val parameters=args.trim.split(" ")  //Console.readLine()
    // val inputParameters=parameters.trim.split(" ")
    if (args.length != 4) {
      ...
      var Env = system.actorOf(Props(new Master(args(0).trim.toInt, args(1).trim, args(2).trim, args(3).trim.toDouble, system)), name = "Gossip_Environment")
      //println("adadssdad")
      Env ! "Start"
      //System.exit(1)
...
class Master(n: Int, topology: String, algorithm: String, ratio: Double, sys: ActorSystem) extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    case "Start" =>
      {
        context.system.scheduler.scheduleOnce(Duration.create(10, TimeUnit.MILLISECONDS), self, "randomKill")
        //tick = context.system.scheduler.schedule(Duration.create(10, TimeUnit.MILLISECONDS), Duration.create(50, TimeUnit.MILLISECONDS), self,"randomkill")
        no_of_active = n
    ...
    case Failure =>
      ...
      */
    //tick = context.system.scheduler.schedule(Duration.create(10, TimeUnit.MILLISECONDS), Duration.create(50, TimeUnit.MILLISECONDS), self, "randomKill")
    case sendPostStopState(p) => {
      println("Killed actor node" + p)
      // deletedneighbour=new ArrayBuffer[ActorRef]()
      // deletedneighbour=neighbourArray
      // deleted_neighbour=nodes(p)  

    }
    case "reclaim" => { deleted_neighbour ! setNeighbours(deletedneighbour) }

    // context.system.scheduler.scheduleOnce(Duration.create(0, TimeUnit.MILLISECONDS) , self, "randomKill")
    case "randomKill" => {

      for (i <- 0 to (ratio * n).toInt) {
        println("inside random kill")
        r = Random.nextInt(nodes.length)
        if (nodes(r).isTerminated != true)
          nodes(r) ! Kill
        else
          println(nodes(r) + "already dead")
        println("killing node" + r)
        // tick.cancel()
      } //tick.cancel()
    }
    case TerminateNode => {
      ...
      // if(no_of_active==(n-(ratio*n).toInt)) {

      //   println("random kill while these many nodes left"+(n-(ratio*n).toInt))
      /// self ! "randomKill"
      // }
  ...
  var r = 0
  /*override val supervisorStrategy=OneForOneStrategy(10,Duration.create(50, TimeUnit.MILLISECONDS)) {
...
*/
  ...
  var deletedneighbour = new ArrayBuffer[ActorRef]()
  var deleted_neighbour: ActorRef = null
}
...
class pushSumNode(index: Int) extends Actor {
  ...
  override def postStop() = {
    //println("killed "+self.path.name)
    //master ! sendPostStopState(self.path.name.substring(4).toInt)
  }
  def receive = {
    ...
    case SetState => {
      ...
      if (active && neighbourCount > 0 && master != null) {
...
class gossipNode extends Actor {
  ...
  def receive = {
    ...
    case SendRumour => {
      if (active && neighbourCount > 0 && rumourCount < 10 && master != null) {

        var nextNode = Random.nextInt(neighbours.length)

        neighbours(nextNode) ! Rumour
        //  println("rumour from " + self.path.name + "->" + neighbours(nextNode).path.name)
    ...
    case NodeTerminated =>
      {
        ...
        if (neighbourCount <= 0) { //active=false  
          for (i <- 0 to neighbours.length - 1)
            neighbours(i) ! NodeTerminated
          //  master !TerminateNode
          //println(self.path.name+" is inactive")
        }
        ///println("current neighbours"+neighbours) 
      }
